...
#Adjudicamos que el valor que nos pasan ya esta declarado, es decir, todas las variables son globales, y al definir una funcion, sino existe el id, entonces se crea, los arreglos se definen desde un principio el tamaño, los argumentos de las funciones deben ser iguales a los definidos, solo acepta arreglo unidimensional
# int suma = main(valor); por ahora no se puede hacer esto, ya que no me valida el valor de retorno, me mandara un error semantico
#Tambien recordale que hay operaciones de enteros que resultan tipo int, cosa a tener en cuenta
#Otra cosa para tener en cuenta, cuando se esta haciendo operacion con entero la division entre enteros da float.
...
int x = 8;
    ...
    int w = 5;
    int v = 30;
int resultado = ((x < y && z > w) || (v < x && y != z) || !(w == 5));
    '''
